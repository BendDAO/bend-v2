...
contract YieldEthStakingEtherfi is YieldEthStakingBase {
  ...
  function unstake(uint32 poolId, address nft, uint256 tokenId, uint256 unstakeFine) public whenNotPaused nonReentrant {
    ...
    sd.withdrawReqId = liquidityPool.requestWithdraw(address(this), sd.withdrawAmount);
  ...
  function repay(uint32 poolId, address nft, uint256 tokenId) public whenNotPaused nonReentrant {
    ...
    // withdraw eth and repay if possible
    vars.isFinalized = withdrawRequestNFT.isFinalized(sd.withdrawReqId);
    require(vars.isFinalized, Errors.YIELD_ETH_STETH_WITHDRAW_NOT_READY);

    vars.claimedEth = address(this).balance;
    withdrawRequestNFT.claimWithdraw(sd.withdrawReqId);
  ...
  function _getNftYieldInEth(YieldStakeData storage sd) internal view returns (uint256, uint256) {
    uint256 eEthAmount = convertToYieldAssets(sd.yieldAccount, sd.yieldShare);
    uint256 eEthPrice = getEETHPriceInEth();
    return (eEthAmount, eEthAmount.mulDiv(eEthPrice, 10 ** eETH.decimals()));
  }

  function getEETHPriceInEth() internal view returns (uint256) {
